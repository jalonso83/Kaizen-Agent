...
1. `get_kpis` para el periodo. Los datos de campañas PAGADAS están en
   `acquisition.by_source[]` (source, campaign, visitors, leads, registrations,
...
> devuelve los **broadcasts internos** de FinZen (push y slots a usuarios que ya
> están adentro) con su holdout y su `lift_pts`. Son otro objeto y otra
> población. Para leer esas, el skill es `diseno-experimentos`.
