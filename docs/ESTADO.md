...
- [x] Backstop de código para la regla 9 — **hecho 2026-09-17**: `propose_campaign` verifica (`verificarLecturaCerebro`) que en la misma conversación haya al menos una llamada a `search_cerebro` con resultado, releyendo el audit log como el backstop de la regla 1. Sin ella, rechaza con la instrucción de buscar decisions-log y estado actual. Una búsqueda vacía no cuenta. Probado en `npm test`; protocolo por chat en `TESTING.md` §11b. Con esto quedan cerrados los tres hallazgos (B) de la auditoría del 2026-08-07
